/* src/components/vendor/pages/VendorVerificationPage.jsx */
...
export default function VendorVerificationPage({ vendor, reviews, onNavigate }) {
  ...
  if (isFullyVerified) return (
    <div className="vd-section">
      <div className="vd-section-head"><h2>Verification</h2></div>
      <div className="vd-verified-card">
        <div style={{ display: "flex", gap: 12 }}>
          <Rosette size={48} />
          <Rosette blue size={48} />
        </div>
  ...
  if (isPlatform && !isCommunity) return (
    <div className="vd-section">
      <div className="vd-section-head"><h2>Verification</h2></div>
      <div className="vd-verified-card" style={{ borderColor: "var(--mint)" }}>
        <Rosette size={44} />
        <div className="vd-verified-title">Platform Verified ✓</div>
        <div className="vd-verified-sub">You have the teal platform badge. Now work toward your Community Verified badge by collecting buyer reviews.</div>
      </div>
      <CommunityProgress reviewCount={reviewCount} avgRating={avgRating} progress={communityProgress} target={communityTarget} onNavigate={onNavigate} />
    </div>
  );
  ...
  return (
    <div className="vd-section">
      ...
      {/* Benefits */}
      <div style={{ fontFamily: "'Sora',sans-serif", fontSize: 13.5, fontWeight: 700, color: "var(--text)" }}>
        Why get verified?
      </div>
      ...
      {/* Two paths */}
      <div style={{ fontFamily: "'Sora',sans-serif", fontSize: 13.5, fontWeight: 700, color: "var(--text)", marginTop: 4 }}>
        Choose your path
      </div>
      <div className="vd-verif-options">
        <div className="vd-verif-option" onClick={() => {}}>
          <div className="vd-verif-option-icon" style={{ background: "#e6f5f0" }}><ShieldIco style={{ color: "var(--teal)" }} /></div>
          ...
          <button className="vd-verif-btn teal" onClick={() => onNavigate("EditProfile")}>
            Complete My Profile →
          </button>
        </div>

        <div className="vd-verif-option" onClick={() => {}}>
          <div className="vd-verif-option-icon" style={{ background: "#dbeafe" }}><CardIco style={{ color: "#1d4ed8" }} /></div>
          <div className="vd-verif-option-title">Get Community Verified</div>
          <div className="vd-verif-option-desc">Share your business card on social media and WhatsApp. Collect 10+ reviews with 4★ avg to earn the blue community badge automatically.</div>
          <CommunityProgress reviewCount={reviewCount} avgRating={avgRating} progress={communityProgress} target={communityTarget} onNavigate={onNavigate} inline />
        </div>
      </div>
    </div>
  );
...
function CommunityProgress({ reviewCount, avgRating, progress, target, onNavigate, inline }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: inline ? 0 : 0 }}>
      ...
      <button className="vd-verif-btn outline" style={{ marginTop: 4 }} onClick={() => onNavigate("BusinessCard")}>
        Generate & Share Business Card →
      </button>
    </div>
  );
}